...
import { BaseCheckpointSaver, TASKS, copyCheckpoint, } from "@langchain/langgraph-checkpoint";
...
function prepareSql(db, checkpoint_id) {
    const sql = `
  SELECT
    thread_id,
    checkpoint_ns,
  ...
  FROM checkpoints
  WHERE thread_id = ? AND checkpoint_ns = ? ${checkpoint_id
        ? "AND checkpoint_id = ?"
...
CREATE TABLE IF NOT EXISTS writes (
  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
    ...
    async getTuple(config) {
        ...
        const pending_sends = await Promise.all(JSON.parse(row.pending_sends).map((send) => this.serde.loadsTyped(send.type ?? "json", send.value ?? "")));
        const checkpoint = {
